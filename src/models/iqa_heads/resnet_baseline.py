"""ResNet-50 baseline IQA head: GAP(layer4) → MLP → tanh to MOS range."""
...
class ResNetBaselineIQA(nn.Module):
    def __init__(self, hidden: int = 512, mos_min: float = 0.0, mos_max: float = 100.0):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        feats = self.backbone(x)["layer4"]
        z = self.pool(feats).flatten(1)
        y = self.head(z)
        y = torch.tanh(y)
